...
def process_frame(image_bgr, buffer):
    ...
    hsv_mask = get_mask_from_hsv(h=im_hsv[:, :, 0], s=im_hsv[:, :, 1], v=im_hsv[:, :, 2])
    hsv_mask = (hsv_mask * 255).astype(np.uint8)
    contours, hierarchy = find_contours(noise_suppression(hsv_mask))
...
def action(args):
    ...
    assert all(x < 256 for x in max_values), "All max values have to be lower than 255"
    ...
    proc_video(innput, output, 10, process_frame, (buffer,))
...
def parse_arguments():
    ...
    # Add parser output.
    parser.add_argument("--output", default=None, help="Set the the processed video output path")
